...
|          **`fallbackToSystemTrustStore`** | When using SSL/TLS, fallback to the system's default trust store instead of throwing an error. This is useful for environments where the certificate path might not be available but the system trust store contains the necessary certificates.                                                                                                                                                                                                  |             *boolean*              |        false        |
...
|            **`fallbackToSystemKeyStore`** | When using SSL/TLS with client certificates (mutual authentication), fallback to the system's default key store instead of throwing an error. This is useful for environments where the certificate path might not be available but the system key store contains the necessary certificates.                                                                                                                                                          |             *boolean*              |        false        |
...
|                     **`maxAllowedPacket`** | Maximum size (in bytes) of a packet the driver sends or accepts. When set, the value is advertised to the server during the handshake, commands larger than it are rejected before being sent, and oversized packets received from the server are rejected. When unset, the driver applies no limit to what it sends, and accepts packets up to a quarter of the JVM max heap, clamped between 16Mb and 1Gb. Until authentication completes a fixed 1Mb cap always applies to received packets.                |             *integer*              |                     |
