...
public class MariadbOutParameters implements org.mariadb.r2dbc.api.MariadbOutParameters {
  ...
  public MariadbOutParameters(
    ...
    columnNumber = meta.getParameterMetadatas().size();
  ...
  public <T> T get(int index, Class<T> type) {
    ...
    ColumnDefinitionPacket column = meta.getParameterMetadata(index);
  ...
  public void setPosition(int newIndex) {
    ...
    for (; index < newIndex; index++) {
      if ((nullBitmap[(index + 2) / 8] & (1 << ((index + 2) % 8))) == 0) {
        // skip bytes
        switch (meta.getParameterMetadata(index).getDataType()) {
          case BIGINT:
          case DOUBLE:
            buf.skipBytes(8);
            break;
