...
public class ShortParseTest extends BaseTest {
  ...
  private void longValue(MariadbConnection connection) {
    ...
    connection
        ...
        .flatMap(r -> r.map((row, metadata) -> Optional.ofNullable(row.get(0, Long.class))))
        .as(StepVerifier::create)
        .expectNext(Optional.of(0L), Optional.of(1L), Optional.of(65535L), Optional.empty())
        .verifyComplete();
  }
  ...
  private void floatValue(MariadbConnection connection) {
    ...
    connection
        ...
        .flatMap(r -> r.map((row, metadata) -> Optional.ofNullable(row.get(0, Float.class))))
        .as(StepVerifier::create)
        .expectNext(Optional.of(0F), Optional.of(1F), Optional.of(65535F), Optional.empty())
        .verifyComplete();
  }
  ...
  private void doubleValue(MariadbConnection connection) {
    ...
    connection
        ...
        .flatMap(r -> r.map((row, metadata) -> Optional.ofNullable(row.get(0, Double.class))))
        .as(StepVerifier::create)
        .expectNext(Optional.of(0D), Optional.of(1D), Optional.of(65535D), Optional.empty())
        .verifyComplete();
  }
  ...
  private void decimalValue(MariadbConnection connection) {
    ...
    connection
        ...
        .expectNext(
            Optional.of(BigDecimal.ZERO),
            Optional.of(BigDecimal.ONE),
            Optional.of(BigDecimal.valueOf(65535)),
            Optional.empty())
        .verifyComplete();
  }
